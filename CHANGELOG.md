...
[docs/design/m7-directed-graphs.md](docs/design/m7-directed-graphs.md)）の完了リリース。v1.0 に
向けたプレリリースとして公開する。D1（主用途 = 経路列挙・数え上げ）の有向側を埋め、一方通行の
ある道路網や依存関係グラフの経路列挙・有向単純閉路・有向ハミルトンパス／閉路・有向次数制約・
arborescence（根つき有向全域木）が書けるようになった。**既存 API に破壊的変更は無い**
（`Graph` / `Edge` / `GraphSet` / 既存スペックのシグネチャは一切変えていない）。次の M8 は
安定化と公開（v1.0、API 凍結）。
...
- `DirectedCycleSpec` / `DirectedHamiltonianPathSpec` / `DirectedHamiltonianCycleSpec`: 有向単純閉路・
  ...
  部分集合であることも検証した。
- `DirectedDegreeConstraintSpec` / `ArborescenceSpec`: 有向次数制約と根つき有向全域木（arborescence）
  （M7-5、issue #156、[docs/design/m7-directed-graphs.md](docs/design/m7-directed-graphs.md) §3.4）。
  `DirectedDegreeConstraintSpec` は無向版 `DegreeConstraintSpec` と作りは同じで、フロンティア頂点ごとの
  状態が単一の次数カウントから（入次数, 出次数）の 2 本に増えるだけ——弧 `u→v` は常に `u` の出次数と
  `v` の入次数だけを動かし、両端点の両方向を同時に動かすことは無いため、枝刈り用の「残り候補辺数」も
  方向ごとに独立して前計算できる。`ArborescenceSpec` は設計の分解方針どおり「無向として全域木」＋
  「`root` 以外の入次数がちょうど 1、`root` の入次数 0」で組んだ——前半は `SpanningTreeSpec` が使う
  `SpanningComponentState` の comp 配列をそのまま流用し、後半は 0/1 の「既に親を持つか」フラグ 1 個で
  済む（上下限が任意区間の `DegreeConstraintSpec` と違い固定の 0 か 1 なので、`remaining` を使った
  枝刈りが要らない）。木構造（頂点数 `n`、辺数 `n-1`、非巡回）と各頂点の入次数上限 1 が揃うと、
  入次数の総和が `n-1` である以上「入次数 0 の頂点はちょうど 1 個」が単純な数え上げで確定する——
  `root` の入次数を 0 に固定すればそれがその 1 個になり、他の全頂点の入次数がちょうど 1 になることも
  副産物として保証される（`root` への流入弧が拒否されること、`root` から到達不能な頂点を含むグラフで
  `spanning: true` が空族になることも、追加の到達可能性チェック無しにこの数え上げだけから従う）。
  `spanning: false`（`ForestSpec` が `SpanningTreeSpec` を緩めるのと同じ方向の緩和で、閉じる連結成分の
  個数・時期を問わなくなる）だけは追加の作り込みが要る——弧を 1 本も得ていない孤立頂点はいつ閉じても
  構わないが、弧を得た「本物の木」が `root` を含まずに閉じたら、それは `root` を根とする 1 本の木では
  なく無関係な 2 本目の木になってしまうため。これを弾くのに `ArborescenceComponentState`（内部型）が
  comp 配列と同じ添字で「この連結成分は `root` を含むか」「弧を 1 本でも得たか」の 2 ビットを代表元の
  スロットにだけ意味を持たせる形で運び、合流・退去のたびに引き継ぐ。受け入れ条件として、**有向版
  行列木定理**（有向ラプラシアン `D_in - A^T` の `root` 行・列を除いた余因子）で独立計算した値との
  一致を格子・完全グラフ・ランダムな到達可能グラフで検証し、`Bidirected(g)` 上の arborescence 数が
  root の選び方によらず `g` の全域木数と一致すること（root を固定すれば各全域木の向きが一意に決まる
...
- 対象外（D8、issue #159・#160）: 自己ループ・多重辺（同じ `u → v` が 2 本。逆平行の
  `u → v` / `v → u` は多重辺扱いしない）・混合グラフ（有向辺と無向辺の混在）・DAG 部分グラフの
  列挙・Graphillion 互換 I/O の有向対応。詳細は
  [docs/design/m7-directed-graphs.md](docs/design/m7-directed-graphs.md) §1 のスコープ表を参照
- 有向のベンチ基準値（`Bidirected` 双方向格子の無向比、一方通行混在格子、`K_n` の有向ハミルトン
  閉路、arborescence）は `docs/benchmarks.md` の M7-8 節に記録した（issue #159）。**性能目標は
  置かない**——有向は同じ頂点数でも変数（弧）の数がおよそ無向の 2 倍になるため、無向と同等の
  速度が出る保証は無く、「どれくらい遅くなるか」を測って記録することが目的
