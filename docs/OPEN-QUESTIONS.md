...
| B2 | **P0** | **BDD も扱うか**（未回答） | TdZdd は BDD/ZDD 両対応。BDD も持つとノード表を共有でき応用も広がるが、演算セットが倍になる | **ZDD 専用**。BDD が要るなら別ライブラリ | |
| B3 | **P0** | N 分岐（N-ary）DD を将来サポートするか（未回答） | TdZdd は `N` をテンプレート引数に取る。C# でジェネリックにすると全域に波及する | **2 分岐固定**。多値変数は 1-hot 符号化で対応 | |
