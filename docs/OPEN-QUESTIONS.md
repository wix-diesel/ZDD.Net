...
| **B26** | `Zdd` にある同一操作の別名 4 組を残すか | **両方 public のまま残す**。`OnSet` / `OffSet` / `SupersetsOf` / `SubsetsOf` を正、`Subset1` / `Subset0` / `Restrict` / `Permit` を SAPPOROBDD / TdZdd 互換の別名とする（M8-7） | 移植時の入口として互換名が有用で、削除しても `NonSubsetsOf` / `NonSupersetsOf` に互換名がないため API 全体は対称にならない。`[Obsolete]` も付けない。警告を出してまで一本化する理由がなく、`TreatWarningsAsErrors` の利用者には実質的な削除になるため |
