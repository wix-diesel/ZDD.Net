...
- ビームサーチによるパス幅近似最小化（`BeamSearchPathWidth`）は、当初この節が想定していた
  v0.4 ではなく **M3-3（v0.3）に前倒し**して実装済み（[docs/ROADMAP.md](ROADMAP.md) の M3 表）。
- 格子グラフは専用の蛇行順序を用意。
...
8. **並列化**（v0.4、M4-3・issue #46・実装済み）: フロンティア構築のレベル内展開を `Parallel.For` 化。
   当初案の「パーティション別状態表 → 結合」は実測で不採用——状態表への登録
   （`GetOrAdd`）自体がこのライブラリの支配的コストで、パーティション内の重複除去では
   その登録を 2 回（パーティション内 + 結合時）払うコストを取り戻せなかった。採用したのは
   「`GetChild` の計算だけを並列化し、状態表への登録は結合フェーズで単一スレッドがパーティション順に
   1 回だけ行う」案（詳細・実測は docs/benchmarks.md §M4-3）。フロンティア構築は演算キャッシュ
   （`OperationCache`、Core の apply 系演算専用）を使わないので、「演算キャッシュのスレッドローカル
   分割」は該当なし。
