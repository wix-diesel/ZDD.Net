# API レビューメモ（M5-7 → M9-1 引き継ぎ）

> **改番の注記 (2026-09-06)**: 本メモ作成時点で「公開 API の凍結」は M6-1 だったが、
> その後 v0.6「API 拡充と相互運用」・v0.7「有向グラフ対応」、さらに v0.8「API の対称化と
> 凍結準備」を前に挟んだため、**凍結は M9-1（issue #60）まで繰り下がった**。
> 本文中の参照は M9-1 に更新済み。繰り下げた理由は [docs/ROADMAP.md](ROADMAP.md) 末尾の
> 「M8 を差し込んだ理由」「M6 / M7 を差し込んだ理由」を参照。
>
> **本メモの指摘の行き先 (2026-09-06)**: §1（別名 4 組）は **M8-7**、§2（`LongCount` の非対称）は
> **M8-4**、§4（`IHybridDdSpec`）は **M8-6** として issue 化した。§3（2 つの
> `FrontierManager` の語彙）は M9-1 の中で「意図的な差か埋め忘れか」を確定させる宿題として残る。
> 各項目の設計は [docs/design/m8-api-symmetry.md](design/m8-api-symmetry.md) を参照。

> M5-7（v0.5 リリース、issue #59）の完了条件「v1.0 に向けた API レビューの下準備」として、
> public API の一覧を棚卸しし、命名・一貫性の気になる箇所をここに記録する。**実際の凍結・
> 命名変更・`[Experimental]` の確定は M9-1（公開 API の凍結、issue #60）で行う**。このメモは
> 提案であって決定ではない。
...
全 public 型・メンバーを列挙する一回限りのスクリプトで棚卸しした（M9-1 で入る
...
M9-1 での論点: `Zdd.LongCount()` を追加して 3 層で揃えるか、`Zdd` は `BigInteger` のみを
...
ところ無いため)。M9-1 での論点: 2 つのマネージャの API 表を並べて、意図的な差なのか埋め忘れなの
かを確定させる。
...
現時点で `[Experimental]` を付与している public API は**0 件**。M9-1 の完了条件が「`[Experimental]`
