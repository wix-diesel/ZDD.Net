...
**性能目標は置かない。** v0.7 の目的は「有向グラフが書ける」という機能そのものであり、
同じ頂点数でも有向は変数（弧）の数がおよそ無向の 2 倍になるため、無向と同等の速度が出る保証は無い
（[docs/design/m7-directed-graphs.md](design/m7-directed-graphs.md) §4）。ここで記録するのは
「どれくらい遅くなるか」「幅がどれだけ広がるか」という実測値そのものであり、その数値を後の PR が
達成すべき目標として扱う M2-11 の位置づけとは違う（issue #159）。測定環境は本ドキュメント冒頭の表と
同じ構成（クラウドの共有仮想環境）だが、実測日時点の CPU 表記のみ以下のとおり異なる。

| 項目 | 値 |
|---|---|
| CPU | Intel(R) Xeon(R) Processor @ 2.10GHz, 4 論理コア（クラウドの共有仮想環境。上表の M2-11 計測時とは別インスタンスのため、CPU クロック表記が異なる） |
| OS / .NET | 本ドキュメント冒頭の表と同じ（Ubuntu 24.04.4 LTS、.NET SDK 10.0.111 / Runtime 10.0.11） |
| 測定日 | 2026-09-06 |

実行方法:

```bash
dotnet run -c Release --project bench/ZDD.Net.Benchmarks -- directed-graph
```

[bench/ZDD.Net.Benchmarks/DirectedGraphReport.cs](https://github.com/wix-diesel/ZDD.Net/blob/main/bench/ZDD.Net.Benchmarks/DirectedGraphReport.cs)
が実行するのは 4 節構成の設計書 §4 の表そのもの。**どの節も、正当性の照合に失敗すると例外を投げて
止まる**——数値を記録するだけでなく、記録する数値そのものが正しいことをベンチ自身が毎回検証する。

### 双方向格子の s–t 有向パス: 無向との倍率（表 1 行目）

`DirectedGraph.Bidirected(Graph.Grid(n,n))` 上の有向 s–t パス数を、同じ格子の無向パス数と比較する。
無向パスは s→t 方向に一意に向き付けられるので**個数が厳密に一致するはずで**、これは
OEIS A007764（`PathSpecTests.CountMatchesOeisA007764ForDiagonalGridPaths` が既に確認済みの数列）の
検証を有向側でも兼ねる。一致しなければベンチが例外を投げる。

| n | 無向時間 | 有向時間 | 倍率 | 無向幅 | 有向幅 | 倍率 | 無向ノード数 | 有向ノード数 | 倍率 | 無向メモリ | 有向メモリ | 倍率 | 集合数一致 |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|:---:|
| 5 | 33.9 ms | 39.2 ms | 1.16x | 125 | 360 | 2.88x | 546 | 546 | 1.00x | 0.1 MB | 0.2 MB | 2.45x | ✅ |
| 6 | 28.0 ms | 51.5 ms | 1.84x | 428 | 1,466 | 3.43x | 2,142 | 2,142 | 1.00x | 0.2 MB | 1.0 MB | 5.50x | ✅ |
| 7 | 61.8 ms | 256.3 ms | 4.15x | 1,460 | 6,010 | 4.12x | 7,968 | 7,968 | 1.00x | 0.7 MB | 5.8 MB | 8.57x | ✅ |
| 8 | 165.1 ms | 849.6 ms | 5.15x | 5,054 | 25,066 | 4.96x | 28,528 | 28,528 | 1.00x | 2.7 MB | 27.1 MB | 9.93x | ✅ |

- **ノード数の倍率がちょうど 1.00x であること自体が集合数一致の帰結**——無向・有向で表す族が
  本質的に同じなので、同型の ZDD が同じノード数で構築される。
- **時間・幅・メモリの倍率は n が大きくなるほど広がる**（時間 1.16x→5.15x、幅 2.88x→4.96x、
  メモリ 2.45x→9.93x）。弧数が辺数のちょうど 2 倍になることに加え、有向の状態（`DirectedPathSpec`）は
  mate 配列に加えて頂点ごとの向きビットを持つぶん 1 状態あたりの表現も無向より大きく、
  両方の効果が重なって幅・時間の倍率が単純な「変数 2 倍」以上に伸びている。

### 一方通行混在格子: p とフロンティア幅の関係（表 2 行目）

各辺を固定の閾値でランダムに一方通行化する（閾値は p に依らず辺ごとに固定するため、
**p を大きくしたときの弧集合は必ず小さい p の弧集合の部分集合になる**——「p が大きくなるほど
パス数が単調に減る」がその帰結として成り立ち、増加したらベンチが例外を投げる形で検証している）。

| n | p | 弧数 | 時間 | ピークメモリ | 幅 | ノード数 | 集合数 |
|---:|---:|---:|---:|---:|---:|---:|---:|
| 6 | 0.00 | 120 | 21.6 ms | 0.8 MB | 1,466 | 2,142 | 1,262,816 |
| 6 | 0.25 | 110 | 21.3 ms | 0.4 MB | 1,119 | 669 | 36,315 |
| 6 | 0.50 | 92 | 14.3 ms | 0.2 MB | 551 | 53 | 108 |
| 7 | 0.00 | 168 | 66.4 ms | 4.1 MB | 6,010 | 7,968 | 575,780,564 |
| 7 | 0.25 | 151 | 40.9 ms | 1.8 MB | 4,709 | 3,052 | 7,194,606 |
| 7 | 0.50 | 131 | 0.3 ms | 0.0 MB | 0 | 0 | 0 |

- **p が上がるほど幅・時間・メモリ・集合数のすべてが単調に減る**——一方通行化は弧を減らす方向にしか
  働かないので当然だが、n=6 では p=0→0.5 で幅が 1,466→551（約 0.38 倍）、集合数は
  1,262,816→108 まで落ちる。実際の道路網でも一方通行が増えるほど迂回路の組み合わせが指数的に
  減っていくことを裏付ける結果になっている。
- **`n=7, p=0.50` は集合数 0（s–t パスが 1 本も無い）で、正直にそのまま記録する**——この回のランダムな
  向き付けで、格子の角（頂点 0）の 2 本の辺がどちらも「入る」向きに固定されてしまい、
  頂点 0 から弧が 1 本も出せなくなったため。角の次数がちょうど 2 という格子特有の脆さで、
  各辺が独立に「一方通行かつ入る向き」になる確率は p=0.5 のとき 0.5×0.5=0.25、
  2 本の辺が両方ともそうなる確率はその 2 乗でおよそ 1/16（約 6%）——小さくはあるが
  無視できるほど小さくもない。これも「p を振ったときに何が起きるか」の
  実例として記録する価値がある——一方通行の道路網で角の交差点が孤立しうるのは非現実的でもない。
  時間・メモリが 0 に近いのは、頂点 0 が出次数 0 と分かった時点でビルドがほぼ即座に終わるため。

### 辺順序戦略ごとの差

上表で最も厳しいケース（n=7, p=0.50）について、`DirectedGraph.EstimateMaxFrontierSize` で
ビルドなしに幅だけを比較すると `AsGiven = Bfs = BeamSearchPathWidth = 8` で並ぶ
(このケースは頂点 0 が孤立しているため、どの戦略で辺を並べ替えても幅の上限が小さく出るのは
当然の結果——一方通行化前の同じ格子形状で比較するとまだ差が出る余地があるが、
無向側で M3-1/M3-3 節が既に確認済みの「辺順序最適化・ビームサーチが幅を狭める」効果は
`FrontierManager`/`EdgeOrdering` を有向・無向で共有する M7-2 の設計（docs/design/m7-directed-graphs.md
§2.3）どおり有向グラフでもそのまま機能しており、新たに検証すべき有向特有の差は無い)。

### K_n の有向ハミルトン閉路: `(n-1)!` との照合（表 3 行目）

`DirectedHamiltonianCycleSpec` の集合数を `(n-1)!` および無向 `HamiltonianCycleSpec` の集合数の
2 倍の両方と照合する（design doc §3.3: 無向の単純閉路 1 つに向きが 2 通りある）。両方一致しなければ
例外を投げる。

| n | 無向時間 | 有向時間 | 倍率 | 無向幅 | 有向幅 | 倍率 | 無向メモリ | 有向メモリ | 倍率 | 有向集合数 | `(n-1)!` | 一致 |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|:---:|
| 6 | 6.5 ms | 12.5 ms | 1.92x | 52 | 811 | 15.60x | 0.0 MB | 0.1 MB | 3.07x | 120 | 120 | ✅ |
| 7 | 6.5 ms | 26.0 ms | 4.00x | 137 | 3,940 | 28.76x | 0.1 MB | 0.6 MB | 10.65x | 720 | 720 | ✅ |
| 8 | 8.7 ms | 76.9 ms | 8.82x | 395 | 19,951 | 50.51x | 0.1 MB | 3.3 MB | 36.78x | 5,040 | 5,040 | ✅ |
| 9 | 13.0 ms | 321.2 ms | 24.77x | 1,019 | 104,837 | 102.88x | 0.2 MB | 52.4 MB | 272.53x | 40,320 | 40,320 | ✅ |

- **これが 4 ケース中もっとも倍率が急激に伸びるケース**: 幅の倍率が n=6 の 15.6x から n=9 では
  102.9x まで悪化している。`K_n` は密グラフの極限（全頂点対が辺で結ばれる）なので、
  弧数が辺数のちょうど 2 倍になる効果に加えて、ハミルトン閉路は「全頂点を訪問し終える」という
  グローバルな制約を持つため、稠密グラフほど有向の追加状態（向きビット）が効いてくる。
  **これは「密グラフでの限界確認」という設計書 §4 の狙いどおりの結果**——無向側の
  `SpanningTree_Complete8` (幅 406、docs/benchmarks.md 冒頭の表) と同程度の規模でも、
  有向ハミルトンは大幅に重くなることが分かる。

### arborescence: 有向行列木定理との照合（表 4 行目）

`ArborescenceSpec` の集合数を、独立実装した有向行列木定理（Tutte、
[bench/ZDD.Net.Benchmarks/DirectedGraphReport.cs](https://github.com/wix-diesel/ZDD.Net/blob/main/bench/ZDD.Net.Benchmarks/DirectedGraphReport.cs)
の `CountArborescences` — `tests/ZDD.Net.Tests/Harness/Kirchhoff.cs` と同じ定理の、ベンチ専用の
独立した再実装）と照合する。双方向格子では根を固定した arborescence 数が無向の全域木数と厳密に
一致する（design doc §3.4）ので、無向との倍率も同時に取れる。

**双方向格子 vs 無向全域木（root=0）**:

| 格子 | 無向時間 | 有向時間 | 倍率 | 無向幅 | 有向幅 | 倍率 | 無向メモリ | 有向メモリ | 倍率 | 集合数一致 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|:---:|
| 2×3 | 5.3 ms | 6.8 ms | 1.28x | 5 | 24 | 4.80x | 0.0 MB | 0.0 MB | 1.07x | ✅ |
| 3×3 | 3.1 ms | 6.2 ms | 2.02x | 5 | 36 | 7.20x | 0.0 MB | 0.0 MB | 1.17x | ✅ |
| 3×4 | 4.3 ms | 8.8 ms | 2.07x | 14 | 172 | 12.29x | 0.0 MB | 0.1 MB | 1.51x | ✅ |
| 4×4 | 5.9 ms | 13.0 ms | 2.18x | 14 | 176 | 12.57x | 0.0 MB | 0.1 MB | 2.17x | ✅ |

**ランダムな到達可能グラフ vs 有向行列木定理（無向側の対応物は無い）**:

| 頂点数 | 弧数 | 時間 | ピークメモリ | 幅 | ノード数 | 集合数 | 行列木定理 | 一致 |
|---:|---:|---:|---:|---:|---:|---:|---:|:---:|
| 8 | 20 | 7.0 ms | 0.1 MB | 364 | 232 | 188 | 188 | ✅ |
| 10 | 17 | 5.4 ms | 0.1 MB | 432 | 196 | 128 | 128 | ✅ |
| 12 | 19 | 7.0 ms | 0.1 MB | 792 | 109 | 48 | 48 | ✅ |

- 双方向格子の幅の倍率（4.8x〜12.6x）は、根つきの向き情報 1 ビット／頂点が
  `SpanningComponentState` の comp 配列に上乗せされるコストで、`DirectedPathSpec` の
  1.4x〜2.9x（表 1）や `DirectedHamiltonianCycleSpec` の 15.6x〜102.9x（表 3）の中間に位置する——
  全域木は経路よりも「同時に開いていられる成分」の自由度が大きい一方、ハミルトン閉路ほど
  グローバルな制約が強くないため、倍率もその中間に落ち着いている。
- ランダム到達可能グラフのケースは弧数が 20 前後と小さいにもかかわらず、幅がそれぞれ
  364〜792 と格子ケースより広い——`SpanningComponentState` の comp 配列は頂点数に対して
  状態空間が急激に増えるため、疎な弧数でも頂点数（8〜12）なりの組み合わせ爆発が出る。
  格子のような局所構造がないランダムグラフでは、辺順序を最適化しても幅が抑えにくいことの一例
  （docs/frontier-guide.md §4 の注記どおり）。

### まとめ

- **有向は無向のおよそ 1.2x〜25x（時間）・1.4x〜103x（幅）・1.1x〜273x（メモリ）遅い/広い/重い**——
  変数（弧）数が単純に 2 倍になる効果に加え、密グラフ（`K_n`）やグローバル制約の強いスペック
  （ハミルトン閉路）ほど倍率が急激に悪化する。疎で局所構造のある格子の s–t パスがもっとも
  倍率が小さく（1.2x〜5.2x）、密な `K_n` のハミルトン閉路がもっとも大きい（最大 273x、n=9）。
  **これは想定どおりの結果であり、性能目標を達成できなかったという意味ではない**
  （本節冒頭のとおり、そもそも目標を置いていない）。
- 一方通行化の割合 p を上げるほど幅・時間・メモリ・集合数のすべてが単調に減ることを確認した
  （弧集合が p の増加に対して単調に縮小するという構成上の性質からの帰結として、ベンチ自身が検証）。
- 正当性の照合（OEIS A007764 相当・`(n-1)!`・有向行列木定理）はすべてベンチの実行時に検証され、
  一致しなければ例外で落ちる——記録された数値はすべてこの検証を通過したものである。
- 辺順序最適化・ビームサーチ（M3-1/M3-3）は `FrontierManager`/`EdgeOrdering` の共有実装
  （M7-2）を通じてそのまま有向グラフでも機能する。
