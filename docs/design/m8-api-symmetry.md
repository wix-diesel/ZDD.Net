...
`src/ZDD.Net` の4層の public メソッドを突き合わせ、M8-1〜M8-4 の追加と、各層に
意図的に置かない API を反映した最終結果。
...
（`—` は「その層には概念が無い／公開 API として重ねない」、`×` は「あるべきだが無い」。
この表には `×` が残っていない。）

グラフ層では、基本的な集合代数と極大・極小だけを直接公開し、それ以外の高度な族代数は
`Zdd` / `SetSet<T>` に降りるか `FromZdd` で戻す方針とした。グラフ全体をユニバースとする
補集合も `PowerSet` と `Difference` で表現できるため、専用 API は重ねない。
`GraphConstraints` は無向グラフ専用であり、有向版には対応する制約コンテナが無いので
`DirectedGraphSet.Where` は置かない。これらは不足ではなく意図的な `—` である。
...
| `LongCount()` | `Zdd` | `checked((long)Count)`。`docs/api-review-notes.md` §2 の宿題。4 層で揃う |
...
実装後は3 arc の全族 256 通りについて、6メソッドの結果を同じ添字の辺を持つ
`GraphSet` と照合した。対象を絞る版はグラフ外の arc を `ArgumentException` で拒否し、
変換後の族にもフィルタを合成できる。`Zdd.LongCount()` は `SetSet<T>.LongCount()` と一致し、
2^63 個の集合を持つ族では `OverflowException` になる。
