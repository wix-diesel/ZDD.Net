...
### スペックはスレッド間で状態を共有しないこと（M4-3 の並列構築）

`FrontierBuilder` は、幅が十分に広い水準では `GetChild` を複数のスレッドから同時に呼ぶ
（`BuildOptions.MaxDegreeOfParallelism`。既定は論理コア数、`1` で逐次実行に固定できる。
docs/frontier-guide.md §6.3）。これは「同じ `TSpec` の値を複数スレッドが同時に使う」ということであり、
上の「状態の寿命と書き換えてよい範囲」の規約に**もう 1 つ**要件を足す:

- **スペック自身は、呼び出しをまたいで共有される可変フィールドを持たないこと。** 構築器はスペックの
  値そのものを何度もコピーして使うので、`readonly struct` として書いていれば（7 節）値そのものは
  自動的にスレッドごとに独立している。危ういのは**参照型のフィールド**が指す先を `GetChild` の中で
  書き換える場合——構造体をコピーしても参照先は 1 つのままなので、2 つのスレッドが同じオブジェクトを
  同時に書き換えることになる。
- **読むだけの参照型フィールドは問題ない。** グラフの隣接辺配列や、構築前に確定して以後変わらない
  参照テーブルのような「構築の間ずっと同じ値であり続けるもの」は、複数スレッドから同時に読んでも安全。
  このライブラリの組み込みスペック（`Specs/` 以下すべて）はどれもこの形——`Graph` /
  `FrontierManager` / `VertexFrontierManager` と、構築時に 1 度だけ計算する `readonly int[]` の
  補助テーブルだけを持ち、`GetChild` の中で書き換えるフィールドを一切持たない——なので、そのまま
  並列構築に使える。
- 疑わしいときの見分け方: **`GetChild` の中で `this` の外側のフィールドに代入していなければ安全**
  （ローカル変数や `ref`/`Span` で渡された状態への書き込みは、この規約の対象外——それは呼び出しごとに
  独立しているのが前提の、通常の状態遷移そのもの）。
