...
# archive instead. That gives us the .NET 8 SDK, which covers both target
# frameworks of this repository (netstandard2.0 and net8.0).
set -euo pipefail
...
if ! command -v apt-get >/dev/null 2>&1; then
    echo "setup-dev-env: apt-get not found; install the .NET 8 SDK manually." >&2
    exit 0
fi
