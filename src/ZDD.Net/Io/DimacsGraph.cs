...
namespace ZDD.Net.Io
{
    ...
    /// problem line of <c>p arc &lt;vertexCount&gt; &lt;edgeCount&gt;</c> instead of <c>p edge ...</c>
    /// marks the file as directed &#8212; matching the convention of the DIMACS shortest-path (<c>.gr</c>)
    /// family of formats, which uses <c>arc</c> for a directed problem &#8212; and its <c>e &lt;u&gt;
    /// &lt;v&gt;</c> lines then read as the arc <c>u</c> &#8594; <c>v</c>. As with
    /// <see cref="SimpleTextGraph"/>, which of <see cref="Graph"/> or <see cref="Graphs.DirectedGraph"/>
    /// comes back is decided by which method is called: <see cref="Read(TextReader)"/> only ever returns
    /// a <see cref="Graph"/> and rejects a <c>p arc</c> problem line (pointing at
    /// <see cref="ReadDirected(TextReader)"/> instead), while <see cref="ReadDirected(TextReader)"/> only
    /// ever returns a <see cref="Graphs.DirectedGraph"/> and rejects a <c>p edge</c> problem line
    /// (pointing back at <see cref="Read(TextReader)"/>). A plain, pre-existing (undirected) file keeps
    ...
    public static class DimacsGraph
    {
        ...
        public static Graph Read(TextReader reader)
        {
            ...
            while ((line = reader.ReadLine()) is not null)
            {
                ...
                if (tokens[0] == "p")
                {
                    ...
                    if (tokens.Length == 4 && tokens[1] == "arc")
                    {
                        throw new GraphFormatException(
                            lineNumber,
                            "This is a directed DIMACS file ('p arc ...'); use DimacsGraph.ReadDirected instead.");
                    }
        ...
        /// <summary>Reads a directed graph from its DIMACS text representation.</summary>
        /// <param name="reader">
        /// The source text: a <c>p arc &lt;vertexCount&gt; &lt;edgeCount&gt;</c> problem line, then
        /// <c>e &lt;u&gt; &lt;v&gt;</c> lines, each read as the arc <c>u</c> &#8594; <c>v</c>. Comment
        /// lines (<c>c ...</c>), blank lines, extra whitespace, and CRLF/LF line endings are all
        /// tolerated; a missing trailing newline is fine.
        /// </param>
        /// <exception cref="ArgumentNullException"><paramref name="reader"/> is <see langword="null"/>.</exception>
        /// <exception cref="GraphFormatException">
        /// The input has no <c>p arc</c> problem line (including a <c>p edge</c> line, which is an
        /// undirected file &#8212; read it with <see cref="Read(TextReader)"/> instead), a line is
        /// malformed, a vertex index is outside <c>1 .. vertexCount</c> (DIMACS is 1-based), an arc is a
        /// self-loop, an arc duplicates an earlier arc (the anti-parallel arc <c>v</c> &#8594; <c>u</c> of
        /// an arc <c>u</c> &#8594; <c>v</c> is not a duplicate), or the number of <c>e</c> lines does not
        /// match the count declared on the problem line.
        /// </exception>
        public static DirectedGraph ReadDirected(TextReader reader)
        {
            ThrowHelper.ThrowIfNull(reader, nameof(reader));
            ...
            while ((line = reader.ReadLine()) is not null)
            {
                ...
                if (tokens[0] == "p")
                {
                    ...
                    if (tokens.Length == 4 && tokens[1] == "edge")
                    {
                        throw new GraphFormatException(
                            lineNumber,
                            "This is an undirected DIMACS file ('p edge ...'); use DimacsGraph.Read instead.");
                    }

                    if (tokens.Length != 4 || tokens[1] != "arc")
                    {
                        throw new GraphFormatException(lineNumber, "Expected a problem line of the form 'p arc <vertexCount> <edgeCount>'.");
                    }
            ...
            if (edges is null)
            {
                throw new GraphFormatException(lineNumber, "Missing problem line ('p arc <vertexCount> <edgeCount>').");
            }
        ...
        /// <summary>Reads a directed graph from its DIMACS text representation. Convenience wrapper around <see cref="ReadDirected(TextReader)"/>.</summary>
        /// <param name="text">The DIMACS text.</param>
        /// <exception cref="ArgumentNullException"><paramref name="text"/> is <see langword="null"/>.</exception>
        /// <exception cref="GraphFormatException">See <see cref="ReadDirected(TextReader)"/>.</exception>
        public static DirectedGraph ReadDirected(string text)
        {
            ThrowHelper.ThrowIfNull(text, nameof(text));
        ...
        /// <summary>Returns a directed graph's DIMACS text representation. Convenience wrapper around <see cref="WriteDirected(DirectedGraph, TextWriter)"/>.</summary>
        /// <param name="graph">The graph to write.</param>
        /// <exception cref="ArgumentNullException"><paramref name="graph"/> is <see langword="null"/>.</exception>
        public static string WriteDirected(DirectedGraph graph)
        {
            using StringWriter writer = new StringWriter(CultureInfo.InvariantCulture);
            WriteDirected(graph, writer);
            return writer.ToString();
        }

        /// <summary>
        /// Writes a directed graph in DIMACS text format to <paramref name="writer"/>, using a
        /// <c>p arc</c> problem line (matching the <c>.gr</c>-family convention for a directed problem)
        /// and one <c>e &lt;from&gt; &lt;to&gt;</c> line per arc.
        /// </summary>
        /// <param name="graph">The graph to write.</param>
        /// <param name="writer">The destination.</param>
        /// <exception cref="ArgumentNullException"><paramref name="graph"/> or <paramref name="writer"/> is <see langword="null"/>.</exception>
        public static void WriteDirected(DirectedGraph graph, TextWriter writer)
        {
            ThrowHelper.ThrowIfNull(graph, nameof(graph));
            ThrowHelper.ThrowIfNull(writer, nameof(writer));
            ...
            {
                writer.Write("e ");
                writer.Write((edge.From + 1).ToString(CultureInfo.InvariantCulture));
                writer.Write(' ');
                writer.Write((edge.To + 1).ToString(CultureInfo.InvariantCulture));
                writer.Write('\n');
            }
        }
    }
