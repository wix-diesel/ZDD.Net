...
{
    /// <summary>
    /// The family of vertex covers of a graph: vertex sets that include at least one endpoint of every
    /// edge.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// <see cref="VertexFrontierManager.Slot"/> assigns it — the complement of <see cref="IndependentSetSpec"/>'s
    /// state, and indeed a set covers every edge exactly when its complement is an independent set.
    /// </para>
    /// <para>
    /// <b>Per vertex</b> <c>v</c>: if <c>v</c> is excluded, reject it if any already-decided lower-indexed
    /// neighbor is also excluded (that edge would be left with neither endpoint chosen), otherwise accept.
    /// Once <c>v</c>'s highest-indexed neighbor has been decided, its slot is no longer needed by any
    /// future vertex; it is reset so a slot later reused by another vertex never inherits a stale flag.
    /// </para>
    /// </remarks>
    public readonly struct VertexCoverSpec : IArrayDdSpec
    {
        ...
        /// <summary>Creates a spec for vertex covers of <paramref name="graph"/>.</summary>
        /// <param name="graph">The graph to search.</param>
        /// <exception cref="ArgumentNullException"><paramref name="graph"/> is <see langword="null"/>.</exception>
        public VertexCoverSpec(Graph graph)
        {
        ...
        public int GetChild(Span<int> state, int level, int value)
        {
            ...
            if (value == Excluded)
            {
                ...
                {
                    if (state[earlierNeighborSlots[i]] == Excluded)
                    {
                        return DdResult.False; // an already-excluded neighbor: this edge is left uncovered
                    }
                }
            }
